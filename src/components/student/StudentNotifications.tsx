...
export const StudentNotifications: React.FC = () => {
  const { activeStudent, notifications, showToast } = useApp();

  if (!activeStudent) return null;

  const myNotifs = dataService.getNotifications(activeStudent.id);
  ...
  const handleClearAll = () => {
    dataService.clearAllNotifications(activeStudent.id);
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs p-6 space-y-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between pb-4 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Stay updated on task approvals, revisions, and client messages.</p>
        </div>
      ...
      {myNotifs.length === 0 ? (
        <div className="p-12 text-center text-slate-400 text-xs">
          ...
          <h3 className="font-semibold text-slate-700 text-sm">No new notifications</h3>
          <p className="mt-1">When clients update tasks or accept applications, notifications will arrive here.</p>
        </div>
      ) : (
